...
/*   Updated: 2024/08/31 16:40:55 by tookops          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_calculate_texture(t_cub *cub, t_draw_wall *draw_wall)
{
    ...
    draw_wall->step = 1.0 * cub->texture->height[draw_wall->dir] / draw_wall->line_height;
    draw_wall->tex_pos = (draw_wall->y_start - WIN_HEIGTH / 2 + draw_wall->line_height / 2) * draw_wall->step;
}
...
void ft_draw_wall(t_cub *cub, int x, t_draw_wall *draw_wall)
{
	...
	{
		draw_wall->tex_y = (int)(draw_wall->tex_pos) % cub->texture->height[draw_wall->dir];
		draw_wall->tex_pos += draw_wall->step;
		draw_wall->color = *(int *)(cub->texture->addr[draw_wall->dir] + (draw_wall->tex_y * cub->texture->line_len[draw_wall->dir] + draw_wall->tex_x * (cub->texture->bpp[draw_wall->dir] / 8)));
	...
	while (y < WIN_HEIGTH)
	{
		if (y < draw_wall->y_start) // ceiling
			ft_mpp(&cub->data->img, x, y, BLACK_COLOR);
		else if (y >= draw_wall->y_end) // floor
